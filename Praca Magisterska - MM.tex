...
gdzie $y_x \ \left\{0, 1\right\}$ to prawid³owa klasa przyk³adu $x\in\mathcal{X}$.
Funkcja ta okaza³a siê dalece bardziej skuteczna od klasycznego b³êdu ¶redniokwadratowego oraz posiada teorioinformacyjn± interpretacjê jako tzw. odleg³o¶æ \textit{Kullbacka-Leiblera} pomiêdzy rozk³adem wyznaczonym przez $f_{\theta}$, a uczony rozk³adem pochodz±cym z danych (\cite{kullback-leibler}). Innym okre¶leniem logistycznej funkcji kosztu jest \textit{binarna entropia krzy¿owa} (ang. \textit{binary crossentropy}).

\subsubsection{Uogólnienie funkcji \textit{log-loss} do zagadnienia multiklasyfikacji}

Istnieje naturalne przed³u¿enie regresji logistycznej do zagadnienia multiklasyfikacji zwane regresj± multinomialn±. W tym wypadku zamiast funkcjona³u u¿ywamy wielowymiarowego przekszta³cenia liniowego, miast funkcji sigmoidalnej u¿ywa siê tzw. funkcji \textit{softmax} $Soft: \mathbb{R}^n\rightarrow\mathbb{R}^n$, zadanej wzorem:

$$Soft_\theta(x)_i = \frac{e^{(\theta_i x)_i}}{\sum_{j = 1}^{n}e^{(\theta_j x)_j}},$$

gdzie $\theta$ to macierz przekszta³cenia liniowego, natomiast funkcja kosztu (nazywana \textit{kategoryczn± entropi± krzy¿ow±} - ang. \textit{categorical crossentropy} - lub \textit{entropi± krzy¿ow± typu softmax} - ang. \textit{softmax crossentropy}) zadana jest wzorem:
...
Przedstawiona przez nas definicja konwolucyjnych sieci neuronowych jest bardzo ogólna. W szczególno¶ci nie do koñca jasnym mo¿e byæ sk±d wziê³o siê okre¶lenie sieci jako \textit{konwolucyjnej}. Pewnym wyt³umaczeniem mog± byæ poni¿sze przyk³ady na poni¿szym obrazku przedstawiaj±ce dwa historyczne przyk³ady pierwszych topologii konwolucyjnych.
...
Rzutowania te mo¿na zinterpretowaæ jako wycinanie prostok±tów rozmiaru $(f_w, f_h)$ z obrazka. Na parametry $(s_w, s_h)$ mo¿na natomiast spojrzeæ ile kroków nale¿y zrobiæ w lewo ($s_w$) lub w dó³ $(s_h)$ aby wyci±æ kolejny. Dope³nienia determinuj± czy wycinane kwadraty bêd± wycinane tylko i wy³±cznie z macierzy wej¶ciowej (brak dope³nienia) lub dodatkowe $0$ bêd± dopisane dooko³a obrazka / mapy cech w celu utrzymania rozmiaru wyj¶ciowej mapy cech proporcjonalnego do rozmiaru macierzy wej¶ciowej.
...
W wiêkszo¶ci zastosowañ krokiem wiêkszo¶ci warstw jest wektor $(1, 1)$, co przy zastosowaniu dope³nienia identyczno¶ciowego powoduje, ¿e rozmiary $(w, h)$ macierzy wej¶ciowej i wyj¶ciowej s± identyczne. W zwi±zku z tym, ¿e rozmiar $c$ obydwu tych macierzy jest czêsto wiêkszy od $100$ powoduje powa¿ne problemy pamiêciowe przy obliczeniach wykonywanych w sieci. Jednym z rozwi±zañ tego problemu jest zastosowanie tzw. \textit{poolingu}. Mo¿emy zinterpretowaæ go jako filtr konwolucyjny dla którego zachodzi $(f_w, f_h) \sim (s_w, s_h)$. Powoduje to $(s_w * s_h)$-krotne zmniejszenie rozmiaru macierzy wyj¶ciowej. Pozwala to na znacz±ce ograniczenie pamiêci potrzebnej do wykonania obliczeñ.

Dwa najczê¶ciej spotykane rodzaje \textit{pooling} to tzw. \textit{max-pooling} w którym funkcj± aktywacj± jest funkcja $\max$ oraz tzw. \textit{sum-pooling} - gdzie jako aktywacjê stosujê siê sumê po wszystkich elementach rzutowania.
...
Spo³eczno¶æ badaczy sieci konwolucyjnych jest w dzisiejszych czasach bardzo szeroka i niesposób opisaæ wszystkie trendy i rozwi±zania stosowane przy ich projektowaniu i wykorzystywaniu. Dlatego w tej sekcji przyjrzymy siê najwa¿niejszym historycznie sieciom wraz z analiz± ich najwa¿niejszych rozwi±zañ.

\subsection{Pierwsze sieci konwolucyjne}

Biologicz± podstawê dzia³ania konwolucyjny sieci neuronowych odkryto i sformu³owano ok. 1968r. gdy odkryto, ¿e widzenie odbywa siê przy pomocy tzw. pól receptywnych (ang. \textit{receptive fields}) (\cite{receptive-field}). Jednak za prawdziw± datê ich powstania uznajê siê 1986r. i pracê (\cite{backprop}) (\textit{nota bene} tê sam± w której zdefiniowano \textit{wsteczn± propagacjê}). Przez wiele lat jednak brak dostatecznie du¿ych zbiorów danych oraz mocy obliczeniowej nie pozwala³ sieciom konwolucyjnym na pokazanie pe³nego potencja³u. Chlubnym wyj±tkim by³a sieæ LeNet (\cite{lenet}) autorstwa Yana LeCuna która dziêki wsparciu biologów pracuj±cych nad percepcj± wzrokow± w mózgu by³a w stanie uzyskaæ satysfakcjonuj±ce rezultaty.
\begin{figure}
\centering 
...
\section{Opis eksperymentów i sieci}
