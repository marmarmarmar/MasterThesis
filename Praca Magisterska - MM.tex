...
Na poni¿szych obrazkach mo¿emy zobaczyæ efekt dzia³ania tzw. algorytmu \textit{k-¶rednich}. W tym przypadku przez rodzinê modeli $\Theta_{n}$ bêdziemy uznawaæ n-elementowe krotki kolorów RGB, a przez $\theta_\mathcal{X}$ te z nich które spe³niaj± w³asno¶æ, ¿e je¶li w ka¿dym z pikseli naszego obrazka, zamienimy jego kolor na jeden z wystêpuj±cych w naszej krotce, to suma kwadratów odleg³o¶ci euklidesowych pomiêdzy zanurzeniami faktycznych kolorów pikseli w trójwymiarowej przestrzeni RGB (czyli \textit{de facto} $[0,1]^3$), a zanurzeniem zastêpuj±cych ich kolorów z krotki) bêdzie najmniejsza z mo¿liwych. Poni¿sze obraz 
